...
async def newproduct_command(
    ...
    await state.set_state(
        ProductStates.waiting_for_title
    )
...
async def product_title_handler(
    message: Message,
    state: FSMContext,
):
...
async def product_description_handler(
    ...
    await state.set_state(
        ProductStates.waiting_for_price
    )
...
async def product_price_handler(
    ...
    await state.set_state(
        ProductStates.waiting_for_currency
    )
...
async def product_currency_handler(
    message: Message,
    state: FSMContext,
):
    ...
    await state.set_state(
        ProductStates.waiting_for_file
    )

    await message.answer(
        "Send product file or image"
    )
...
async def product_file_handler(
    message: Message,
    state: FSMContext,
):
